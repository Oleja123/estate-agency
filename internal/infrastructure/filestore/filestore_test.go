...
// minimal PNG signature bytes are enough for filetype to detect PNG
...
func TestSaveToDir_WithAndWithoutExtension(t *testing.T) {
	...
	// without extension - SaveToDir should append detected extension
...
func TestRead_Delete_DeletePropertyDir(t *testing.T) {
	...
	// Delete non-existent path should be nil
	require.NoError(t, fsys.Delete(""))

	// DeletePropertyDir
	// create another file and then remove by property id
	...
	// set basePath to tmp for this fsys instance by creating file under tmp/777
	// FileStore.DeletePropertyDir uses fsys.basePath, so call on id 777
	// ensure basePath points to tmp
	fsys2 := New(tmp)
	require.NoError(t, fsys2.DeletePropertyDir(777))
	require.NoDirExists(t, filepath.Join(tmp, "777"))

	// invalid id should return ErrInvalidInput
